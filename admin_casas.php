<?php
  ...
  if(isset($_POST{'btncerrar'})){
	...
	if(!empty($_POST))
	{
		$valor = $_POST['campo'];
		if(!empty($valor)){
			$where = "WHERE Casa_Id LIKE '%$valor'";
		}
	}
...
<html class="wide wow-animation" lang="es">
  <head>
    <title>Administracion de Casas</title>
    <meta charset="utf-8">
    ...
    <div class="page">
      ...
      <div class="container">
        <div class="row">
          ...
          <a href="casa/casa_add.php" type="button" class="btn btn-primary-12" data-toggle="modal" data-target="#exampleModalCenterCasa">Nuevo Registro</a>
        ...
        <div class="row table-responsive">
          ...
          <tbody>
						<?php while($row = $resultado->fetch_array(MYSQLI_ASSOC)) { ?>
							<tr>
								<td><?php echo $row['Casa_Id']; ?></td>
								<td><?php echo $row['Casa_Calle']; ?></td>
								<td><?php echo $row['Casa_Numero']; ?></td>
                <td><?php $query = $mysqli -> query ("SELECT * FROM Comuna WHERE Comuna_Id='" . $row['Comuna_Id'] . "'");
  
  $valores1 = mysqli_fetch_array($query); echo $valores1['Comuna_Nombre']; ?></td>
                <td><?php $query = $mysqli -> query ("SELECT Region_Nombre FROM Region WHERE Region_Id='" . $valores1['Region_Id'] . "'");
  
  $valores2 = mysqli_fetch_array($query);
echo $valores2['Region_Nombre']; ?></td>
<td><?php $query = $mysqli -> query ("SELECT Usuario_Correo FROM Usuario WHERE Usuario_Id='" . $row['Usuario_Id'] . "'");
  
  $valores3 = mysqli_fetch_array($query);
echo $valores3['Usuario_Correo']; ?></td>
								<td><a href="casa/casa_edit.php?Casa_Id=<?php echo $row['Casa_Id']; ?>"><i class="fa-solid fa-pencil"></i></a></td>
								<td><a href="#" data-href="casa/casa_delete.php?Casa_Id=<?php echo $row['Casa_Id']; ?>" data-toggle="modal" data-target="#confirm-delete"><i class="fa-solid fa-trash"></i></a></td>
                <td><a href="admin_historiales.php?Casa_Id=<?php echo $row['Casa_Id']; ?>"><i class="fa-solid fa-clock-rotate-left"></i></a></td>
                <td><a href="admin_habitacion.php?Casa_Id=<?php echo $row['Casa_Id']; ?>"><i class="fa-solid fa-person-shelter"></i></a></td>
							</tr>
						<?php } ?>
					</tbody>
